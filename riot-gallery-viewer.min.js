...
this.consoleError('option doesn\'t exist',option,value);return!1},initialize(){this.consoleLog('Riot Gallery Viewer - begin initialization of loaded data');const isGalleryWithFileRemoteUrl=this.processGalleryFileRemoteUrls();if(!isGalleryWithFileRemoteUrl){this.initializeRemoteComplete()}},initializeRemoteComplete(){for(let x=0;x<this.galleries.length;x++){const gal=this.galleries[x];if(gal.imageFileUrl&&!gal.imageFileUrlIsComplete&&!gal.isError){return}}
this.buildHtmlGalleries();this.setGalleriesByClass();if(this.options.preloadImagesType==='pageload'){this.preloadAllImages()}},preloadAllImages(){for(let galKey=0;galKey<this.galleries.length;galKey++){this.preloadGalleryImages(galKey)}},preloadGalleryImages(galKey){if(!this.galleries[galKey]){return}
for(let itemKey=0;itemKey<this.galleries[galKey].items.length;itemKey++){this.preloadImage(galKey,itemKey)}},preloadImage(galKey,itemKey){if(!this.galleries[galKey]){return}
...
const img=new Image();img.src=item.url;img.galKey=galKey;img.itemKey=itemKey;this.consoleLog('start image preload',item.url);img.onload=function(e){RiotGalleryViewer.consoleLog('image url preload successful',img.src);RiotGalleryViewer.updateGalItem(this.galKey,this.itemKey,img,!1)}
img.onerror=function(e){RiotGalleryViewer.consoleError('image url preload load error',this.src);RiotGalleryViewer.updateGalItem(this.galKey,this.itemKey,null,!0)}},processGalleryFileRemoteUrls(){let isGalleryWithFileRemoteUrl=!1;for(let x=0;x<this.galleries.length;x++){if(this.galleries[x].imageFileUrl){this.processGalleryFileRemoteUrl(x);isGalleryWithFileRemoteUrl=!0}}
return isGalleryWithFileRemoteUrl},processGalleryFileRemoteUrl(galleryKey){if(!this.galleries[galleryKey]){return!1}
if(!this.galleries[galleryKey].imageFileUrl){return!1}
const xhr=this.createXHR();xhr.open('GET',this.galleries[galleryKey].imageFileUrl,!0);xhr.galleryKey=galleryKey;xhr.onreadystatechange=function(){if(xhr.readyState===4){if(xhr.status===200){RiotGalleryViewer.addGalleryImagesByText(xhr.galleryKey,xhr.responseText)}else{RiotGalleryViewer.galleries[xhr.galleryKey].isError=!0;RiotGalleryViewer.galleries[xhr.galleryKey].errorMessages.push('could not read file: '+xhr.responseURL)}
RiotGalleryViewer.galleries[xhr.galleryKey].imageFileUrlIsComplete=!0;RiotGalleryViewer.initializeRemoteComplete()}};xhr.send()},addGalleryImagesByText(galleryKey,text){let parsed=null;text=this.strReplace(["\r\n","\n\r","\r"],"\n",text);text=text.trim();if(!text){this.galleries[galleryKey].isError=!0;this.galleries[galleryKey].errorMessages.push('addGalleryImagesByText - text file is empty: '+galleryKey);return!1}
const firstChar=text.substring(0,1);const lastChar=text.substring(text.length-1);if(firstChar==='['&&lastChar===']'){parsed=this.parseJson(text);console.log(parsed)}
if(parsed){for(let x=0;x<parsed.length;x++){this.addImageByObj(galleryKey,parsed[x])}}else{const lines=text.split("\n");for(let x=0;x<lines.length;x++){this.addImageByString(galleryKey,lines[x])}}},addImageByObj(galleryKey,obj){if(!this.galleries[galleryKey]){return!1}
...
this.addInitImage(galleryKey,url,thumbUrl,caption);return!0},addImageByString(galKey,line){let url=null;let thumbUrl=null;let caption=null;const strs=line.split("\t");if(strs.length>1){if(strs[0]){url=this.strStripStartEndQuotes(strs[0])}
if(strs[1]){thumbUrl=this.strStripStartEndQuotes(strs[1])}
if(strs[2]){caption=this.strStripStartEndQuotes(strs[2])}
this.addInitImage(galKey,url,thumbUrl,caption)}else{this.addImageByStringsWithQuotes(galKey,line)}},strStripStartEndQuotes(str){const tempQuoteReplace="[~~(quote here, riot gallery)~~]";str=this.strReplace('\\"',tempQuoteReplace,str).trim();if(str.length<1){return''}
const first=str.substring(0,1);if(first==='"'){str=str.substring(1);if(str.length<1){return''}}
const last=str.substring(str.length-1);console.log('1last',last);if(last==='"'){console.log('2str',str);str=str.substring(0,str.length-1);console.log('3str',str);if(str.length<1){return''}}
str=this.strReplace(tempQuoteReplace,'"',str).trim();return str},addImageByStringsWithQuotes(galKey,line){const tempQuoteReplace="[~~(quote here, riot gallery)~~]";line=this.strReplace('\\"',tempQuoteReplace,line);line=line.trim();const strs=line.split('"');let url=null;let thumbUrl=null;let caption=null;if(strs[1]){url=this.strReplace(tempQuoteReplace,'"',strs[1])}
if(strs[3]){thumbUrl=this.strReplace(tempQuoteReplace,'"',strs[3])}
if(strs[5]){caption=this.strReplace(tempQuoteReplace,'"',strs[5])}
this.addInitImage(galKey,url,thumbUrl,caption)},buildHtmlGalleries(){for(let galKey=0;galKey<this.galleries.length;galKey++){if(!this.galleries[galKey].isHtmlBuilt&&!this.galleries[galKey].isError){this.buildHtmlGallery(galKey)}}},buildHtmlGallery(galKey){if(!this.galleries[galKey]){return!1}
...
let isImageAdded=!1;for(let initImageKey=0;initImageKey<this.galleries[galKey].initImgs.length;initImageKey++){if(this.addGalleryLiItemFromInitImage(galKey,initImageKey)){isImageAdded=!0}}
if(!isImageAdded){this.galleries[galKey].isError=!0;this.galleries[galKey].errorMessages.push('addGalleryLiItemsFromInitImages - no images set');return!1}
...
let liElem=document.createElement('li');let aElem=document.createElement('a')
aElem.href=url;aElem.setAttribute('target','_blank');if(!this.setGalleryItem(galKey,url,aElem,caption)){return!1}
...
this.galleries[galKey].elem.appendChild(liElem);return!0},getGalleryKeyByElemId(elemId){if(typeof elemId!=='string'){this.consoleError('cannot add gallery, invalid elemId type',elemId,typeof elemId);return!1}
if(elemId.length<1){this.consoleError('cannot add gallery, elemId is empty',elemId,typeof elemId);return!1}
for(let x=0;x<this.galleries.length;x++){if(this.galleries[x].elemId===elemId){if(this.isError){return!1}
return x}}
let gal=this.getByVal(this.galleryBlank);gal.elemId=elemId;this.consoleLog('new gallery added',elemId);this.galleries.push(gal);const galleryKey=this.galleries.length-1;return galleryKey},getNewGalleryKeyByElem(elem){console.log('getNewGalleryKeyByElem(elem) {',elem);if(!this.getIsElem(elem)){return!1}
for(let k=0;k<this.galleries.length;k++){if(this.galleries[k].elem===elem){if(this.isError){return!1}
return k}}
console.log(this.galleryBlank);let gal=this.getObjByVal(this.galleryBlank);console.log(gal);gal.elem=elem;this.galleries.push(gal);const galKey=this.galleries.length-1;return galKey},addInitImage(galKey,url,thumbUrl,caption){if(!this.galleries[galKey]){return!1}
if(!url||typeof url!=='string'){return!1}
this.galleries[galKey].initImgs.push({url:url,thumbUrl:thumbUrl,caption:caption});return!0},setGalleriesByClass(){const elems=document.getElementsByClassName('riot-gallery');for(let x=0;x<elems.length;x++){this.setGalleryByElem(elems[x],null)}},setGalleryByElem(galElem,galKey){if(galKey===null){galKey=this.getNewGalKeyByElem(galElem);if(galKey===null){return!1}}
...
for(let elemKey=0;elemKey<elems.length;elemKey++){console.log('elemKey',elemKey);this.setGalleryItemByElem(galKey,elems[elemKey])}
...
for(let galKey=0;galKey<this.galleries.length;galKey++){if(this.galleries[galKey].elem===elem){if(this.galleries[galKey].isError){return!1}
return galKey}}
let gal=this.getByVal(this.galleryBlank);gal.elem=elem;this.galleries.push(gal);const galKey=this.galleries.length-1;return galKey},setGalleryItemByElem(galleryKey,elem){const url=this.getImgUrlFromConElem(elem);console.log('p1');if(!url){console.log('p2');return!1}
const clickElem=this.getClickElemFromConElem(elem);if(clickElem){for(let g=0;g<this.galleries.length;g++){for(let i=0;i<this.galleries[g].items.length;i++){if(this.galleries[g].items[i].clickElem===clickElem){console.log('skip element. click elem already set.');return!1}}}}
const caption=this.getCaptionFromConElem(elem);return this.setGalleryItem(galleryKey,url,clickElem,caption)},setGalleryItem(galKey,url,clickElem,caption){if(!this.galleries[galKey]){return!1}
if(!url){return!1}
...
this.galleries[galKey].items.push(item);return!0},getImgUrlFromConElem(conElem){let url;console.log('getImgUrlFromConElem');url=this.getSubElemAttrVal(conElem,'data-riot-gallery-image-url');if(url){return url}
...
if(this.options.useMaterialIcons){this.loadMaterialIcons()}},areMaterialIconsLoaded(){console.log('areMaterialIconsLoaded()');divElem=document.createElement('div');divElem.style.position="fixed";divElem.style.opacity=0;spanElem=document.createElement('span');spanElem.classList='material-symbols-outlined';spanElem.innerHTML='arrow_back_ios_new';divElem.appendChild(spanElem);this.elems.body.appendChild(divElem);const w=divElem.offsetWidth;document.body.removeChild(divElem);if(w<50){this.consoleLog('material icons are already available.');return!0}
return!1},loadMaterialIcons(){console.log('loadMaterialIcons() {');if(this.isMaterialIconsLoadComplete){this.addMaterialIconsToHtml();return}
...
let nextElem=document.getElementById('riot-gallery-viewer-next-con');if(nextElem){let nextAs=nextElem.getElementsByTagName('a');for(let x=0;x<nextAs.length;x++){let spanElem=document.createElement('span');spanElem.classList='material-symbols-outlined';spanElem.innerHTML='arrow_forward_ios';nextAs[x].innerHTML='';nextAs[x].appendChild(spanElem)}}},slideSwipeStartEvent(event){console.log('slideSwipeStartEvent(event)');const temp=this.getSwipeXYFromEvent(event);if(temp===null){this.swipeInfoReset();this.consoleLog('slideSwipeStartEvent - no position found, stop swipe action;');return}
const x=temp[0];this.swipeInfo.startX=x;this.swipeInfo.startTime=this.getCurMs();this.consoleLog('slideSwipeStartEvent - position = '+x)},slideSwipeEndEvent(event){console.log('slideSwipeEndEvent(event)');if(!this.swipeInfo.startX||!this.swipeInfo.startTime){this.swipeInfoReset();this.consoleLog('slideSwipeEndEvent - end swipe with no start swipe, stop swipe action');return}
if(this.options.swipeMaxSeconds){const timeDif=this.getCurMs()-this.swipeInfo.startTime;console.log('this.options.swipeMaxSeconds',this.options.swipeMaxSeconds);console.log('timeDif',timeDif);console.log('if (timeDif > this.options.swipeMaxSeconds * 1000) {',(timeDif>this.options.swipeMaxSeconds*1000),timeDif,this.options.swipeMaxSeconds);if(timeDif>this.options.swipeMaxSeconds*1000){this.swipeInfoReset();this.consoleLog('slideSwipeEndEvent - slide time too long, stop swipe action, max seconds = '+this.options.swipeMaxSeconds+', seconds taken = '+(timeDif/1000));return}}
...
const x=temp[0];const xDif=Math.abs(x-this.swipeInfo.startX);if(this.options.swipeMinPixels||this.options.swipeMinPercent){let swipeDistanceSuccess=!1;if(this.options.swipeMinPixels){console.log('if (xDif >= this.options.swipeMinPixels) {',xDif,this.options.swipeMinPixels);if(xDif>=this.options.swipeMinPixels){swipeDistanceSuccess=!0}
console.log('swipeDistanceSuccess',swipeDistanceSuccess)}
if(!swipeDistanceSuccess&&this.options.swipeMinPercent){const widthPercent=xDif/window.innerWidth*100;console.log('if (widthPercent >= this.options.swipeMinPercent) {',widthPercent,this.options.swipeMinPercent);if(widthPercent>=this.options.swipeMinPercent){swipeDistanceSuccess=!0}
console.log('swipeDistanceSuccess',swipeDistanceSuccess)}
...
const viewer=RiotGalleryViewer.getViewerCur();if(viewer.galKey===this.galKey&&viewer.itemKey===this.itemKey){RiotGalleryViewer.calculateViewerPlacement();RiotGalleryViewer.placeImgInPosition()}}
img.onerror=function(e){RiotGalleryViewer.consoleError('image url load error',this.src);RiotGalleryViewer.updateGalItem(this.galKey,this.itemKey,null,!0);const viewer=RiotGalleryViewer.getViewerCur();if(viewer.galKey===this.galKey&&viewer.itemKey===this.itemKey){RiotGalleryViewer.calculateViewerPlacement();RiotGalleryViewer.placeImgInPosition();RiotGalleryViewer.updateCaption()}}
...
this.galleries[galKey].items[itemKey].width=img.width;this.galleries[galKey].items[itemKey].height=img.height;this.galleries[galKey].items[itemKey].isError=!1;this.galleries[galKey].items[itemKey].isLoaded=!0},endTransition(){if(this.viewerCurKey!==null){let viewer=this.getViewerCur();if(viewer.transition){for(var prop in viewer.transition){if(Object.prototype.hasOwnProperty.call(viewer.transition,prop)){if(prop==='left'){this.elems.imageCons[this.viewerCurKey].style.left=viewer.left+'px'}
if(prop==='top'){this.elems.imageCons[this.viewerCurKey].style.top=viewer.top+'px'}
if(prop==='width'){this.elems.imageCons[this.viewerCurKey].style.width=viewer.width+'px'}
if(prop==='height'){this.elems.imageCons[this.viewerCurKey].style.height=viewer.height+'px'}
if(prop==='opacity'){this.elems.imageCons[this.viewerCurKey].style.opacity=1}}}}
this.viewers[this.viewerCurKey].transition=null}
if(this.transition){clearInterval(this.transition.jsInterval);this.transition=null}
...
newLeft=(percentToStart*viewer.transition[prop])+(percentToEnd*viewer.left);this.elems.imageCons[k].style.left=newLeft+'px'}
if(prop==='top'){let newTop;if(prevViewer){newTop=(percentToStart*prevViewer.top)+(percentToEnd*prevViewer.transition[prop]);this.elems.imageCons[this.viewerPrevKey].style.top=newTop+'px'}
newTop=(percentToStart*viewer.transition[prop])+(percentToEnd*viewer.top);this.elems.imageCons[k].style.top=newTop+'px'}
if(prop==='width'){let newWidth;if(prevViewer){newWidth=(percentToStart*prevViewer.width)+(percentToEnd*prevViewer.transition[prop]);this.elems.imageCons[this.viewerPrevKey].style.width=newWidth+'px'}
newWidth=(percentToStart*viewer.transition[prop])+(percentToEnd*viewer.width);this.elems.imageCons[k].style.width=newWidth+'px'}
if(prop==='height'){let newHeight;if(prevViewer){newHeight=(percentToStart*prevViewer.height)+(percentToEnd*prevViewer.transition[prop]);this.elems.imageCons[this.viewerPrevKey].style.height=newHeight+'px'}
newHeight=(percentToStart*viewer.transition[prop])+(percentToEnd*viewer.height);this.elems.imageCons[k].style.height=newHeight+'px'}
if(prop==='opacity'){let newOpacity;if(prevViewer){newOpacity=(percentToStart*1)+(percentToEnd*0);this.elems.imageCons[this.viewerPrevKey].style.opacity=newOpacity}
...
this.elems.images[curK].src=src;this.elems.imageCons[curK].classList.add('is-displayed')},placeImgInPosition(){console.log('placeImgInPosition() {');const curK=this.viewerCurKey;if(curK===null){return}
...
console.log('if (this.options.transitionType === \'fade\' && isNewTransition) {',this.options.transitionType,isNewTransition);if(this.options.transitionType==='fade'&&isNewTransition){console.log('if (this.options.transitionType === \'fade\' && isNewTransition) {');this.elems.imageCons[curK].style.opacity=0}
...
let val=elem.getAttribute(attr);console.log('let val = elem.getAttribute(attr);',elem,val);if(val!==null&&val!==!1){return val}
...
return this.getGalItem(viewer.galKey,viewer.itemKey)},consoleLog(val1,val2,val3,val4,val5,val6,val7){console.log('consoleLog','if (this.options.doConsoleLog) {',this.options.doConsoleLog);if(this.options.doConsoleLog){if(typeof val7!=='undefined'){console.log(val1,'|',val2,'|',val3,'|',val4,'|',val5,'|',val6,'|',val7)}else if(typeof val6!=='undefined'){console.log(val1,'|',val2,'|',val3,'|',val4,'|',val5,'|',val6)}else if(typeof val5!=='undefined'){console.log(val1,'|',val2,'|',val3,'|',val4,'|',val5)}else if(typeof val4!=='undefined'){console.log(val1,'|',val2,'|',val3,'|',val4)}else if(typeof val3!=='undefined'){console.log(val1,'|',val2,'|',val3)}else if(typeof val2!=='undefined'){console.log(val1,'|',val2)}else if(typeof val1!=='undefined'){console.log(val1)}}
...
if(this.options.doConsoleTrace){console.trace()}},createXHR(){if(window.XMLHttpRequest){return new XMLHttpRequest()}else if(window.ActiveXObject){try{return new ActiveXObject("Msxml2.XMLHTTP")}catch(e){try{return new ActiveXObject("Microsoft.XMLHTTP")}catch(e){}}}
throw new Error("This browser does not support XMLHttpRequest or ActiveXObject")},strReplace(from,to,str){if(typeof from==='number'){from=from.toString()}
...
for(let x=0;x<from.length;x++){let curFrom=from[x];let prev='';while(prev!==str){prev=str;str=str.replace(curFrom,to,str)}}
return str},getByVal(theVar){if(typeof theVar!=='object'){return theVar}
...
return parsed},};window.onload=function(){RiotGalleryViewer.initialize();console.log(RiotGalleryViewer)}